...
- (void)onSubject:(EventSourceEventHandler)handler {
    [self addEventListener:SubjectEvent handler:handler];
}

- (void)onEnd:(EventSourceEventHandler)handler {
    [self addEventListener:EndEvent handler:handler];
}
...
//        for (NSString *line in lines) {
            ...
            if ([msg hasPrefix:ESKeyValueDelimiter]) {
                continue;
            }
            @autoreleasepool {
                ...
                NSString *eventData = [msg componentsSeparatedByString:ESKeyValueDelimiter].lastObject; // subject
                if (key && value) {
...
NSString *const SubjectEvent = @"topic_resp";
NSString *const EndEvent = @"generate_stream_end";
